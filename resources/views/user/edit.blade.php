 <x-app-layout>
        ...
        <div class="relative flex flex-col flex-auto min-w-0 p-4 mx-6 -mt-16 overflow-hidden break-words border-0 shadow-blur rounded-2xl bg-white/80 bg-clip-border backdrop-blur-2xl backdrop-saturate-200">
            <div class="flex flex-wrap -mx-3">
                ...
                <div class="flex-none w-auto max-w-full px-3 my-auto">
                    <div class="h-full">
                        <h5 class="mb-1">Editar Usuário</h5>
                    ...
                    <div class="relative right-0">
                        <ul class="relative flex flex-wrap p-1 list-none bg-transparent rounded-xl" nav-pills role="tablist">
                            <li class="z-30 flex-auto text-center">
                               ...
                               <a href="/manter_usuarios"> <span class="ml-1">Voltar</span></a>     
                            </li>
                        </ul>
                    </div>
                </div>
    ...
    <br>            
    <div class="w-full max-w-full px-3 mx-auto mt-0 md:flex-0 shrink-0 md:w-7/12 lg:w-5/12 xl:w-7/12" style="float: left;">
        <div class="relative z-0 flex flex-col min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
            <div class="p-6 mb-0 text-center bg-white border-b-0 rounded-t-2xl">
                <h5>Edite os dados do usuário :</h5>
            </div>
            ...
            <div class="flex-auto p-6">
                <form method="POST" action="/editar_usuario/{{ $user[0]->id }}" >
                    @csrf
                    <div class="mb-4">
                        <input autofocus id="name" type="text" name="name" value="{{ $user[0]->name }}" required class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="Nome" aria-label="Nome" aria-describedby="email-addon" />
                    </div>
                    <div class="mb-4">
                    ...
                    <div class="mb-4">
                        <select id="permission" name="permission" autocomplete="permission" class="w-full text-sm leading-5.6 focus:shadow-soft-primary-outline ease-soft block rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" aria-label="Email" aria-describedby="email-addon" style="color: #d1d5db;">
                            <option value="" disabled style="color:#d1d5db">Permissão</option>
                            <option value="1">Administrador</option>
                            <option value="2" {{ $user[0]->permissions[0]->name == 'user' ? "selected='selected'" : "" }}  >Usuário Comum</option>
                        </select>
                    </div>
                    <div class="text-center">
                        <x-button class="inline-block w-full px-6 py-3 mt-6 mb-2 font-bold text-center text-white uppercase align-middle transition-all bg-transparent border-0 rounded-lg cursor-pointer active:opacity-85 hover:scale-102 hover:shadow-soft-xs leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 bg-gradient-to-tl from-gray-900 to-slate-800 hover:border-slate-700 hover:bg-slate-700 hover:text-white">Cadastrar</x-button>
                    </div>
                </form>
            </div>
        </div>
